...
class TestIntegration < Minitest::Test
  def test_invalid_top_row
    assert_equal "Invalid: Row 1 had 3 7's", Sudoku::Validator.new(TEST_FILES + '/invalid_top_row.sudoku').validate
  end

  def test_invalid_right_column
    assert_equal "Invalid: Column 9 had 3 7's", Sudoku::Validator.new(TEST_FILES + '/invalid_right_column.sudoku').validate
  end

  def test_invalid_middle_quadrant
    assert_equal "Invalid: Quadrant 5 had 3 7's", Sudoku::Validator.new(TEST_FILES + '/invalid_middle_quadrant.sudoku').validate
  end


  def test_valid
    assert_equal "Valid", Sudoku::Validator.new(TEST_FILES + '/valid_complete.sudoku').validate
  end
